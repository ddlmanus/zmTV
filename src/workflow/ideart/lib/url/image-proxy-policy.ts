...
export function shouldBypassImageProxy(rawUrl: string): boolean {
  ...
  try {
    ...
    if (
      host === "files.tapnow.top" &&
      parsed.pathname.startsWith("/api/conversation/storage/uploads/")
    )
      return true;
    ...
    if (typeof window !== "undefined") {
      const localHost = String(window.location?.hostname || "")
        .trim()
        .toLowerCase();
    ...
    // Accelerated OSS hosts often omit Access-Control-Allow-Origin. Loading them
    // directly with crossOrigin="anonymous" breaks canvas export/readback, so keep
    // them behind our same-origin proxy even if a broad env suffix says otherwise.
    if (
    ...
    const envSuffixes = String(
      process.env.NEXT_PUBLIC_IMAGE_PROXY_BYPASS_HOST_SUFFIXES || "",
    ).trim();
    if (envSuffixes) {
    ...
    if (/^[a-z0-9-]+-\d+\.cos\.[a-z0-9-]+\.myqcloud\.com$/i.test(host))
      return true;
...
function detectBareBase64ImageMime(rawUrl: string): string | null {
  const input = String(rawUrl || "").trim();
  if (!input || input.startsWith("data:") || input.startsWith("blob:"))
    return null;
  ...
  if (input.startsWith("/")) {
    ...
    if (pathLikePrefixes.some((prefix) => lower.startsWith(prefix)))
      return null;
  }
...
export function toImageProxyUrlWithParams(
  rawUrl: string,
  params?: { w?: number | null },
): string {
  const input = maybeRewriteInternalUploadsUrl(rawUrl);
  if (!input) return "";
  if (input.startsWith("data:") || input.startsWith("blob:")) return input;
  ...
  const w = params?.w;
  const wInt =
    typeof w === "number" && Number.isFinite(w) ? Math.floor(w) : null;
...
export function toDesktopRemoteAssetUrl(rawUrl: string): string {
  if (
    typeof navigator === "undefined" ||
    !/electron/i.test(navigator.userAgent)
  )
    return "";
  const input = unwrapImageProxyUrl(rawUrl);
  if (!/^https?:\/\//i.test(input)) return "";
  return `local-asset://remote/${encodeURIComponent(input)}`;
}

export function toDesktopRemoteImageUrl(rawUrl: string): string {
  return toDesktopRemoteAssetUrl(rawUrl);
}
