...
function isXiaohongshuHistoryItem(item: HistoryItem) {
  return (
...
export function HistoryPage() {
  ...
  const xiaohongshuTasks = useXiaohongshuHistoryStore((state) => state.tasks);
  ...
  const pageSize = 50;
  const localXiaohongshuItems = xiaohongshuTasks.map(
    xiaohongshuTaskToHistoryItem,
  );
  const filteredLocalXiaohongshuItems =
    statusFilter === "all"
  ...
  const handleOpenInPlayground = useCallback(
    async (item: HistoryItem) => {
      ...
      const targetRoute = getWorkspaceRoute(workspace, item.model);
      ...
      try {
        ...
        createTab(model, undefined, item.outputs, predictionResult, workspace);
        setSelectedItem(null);
        navigate(targetRoute);
      } finally {
  ...
  const navigateHistory = useCallback(
    (direction: "prev" | "next") => {
      if (!selectedItem || displayItems.length <= 1) return;
      ...
      let newIdx: number;
      if (direction === "prev") {
        newIdx = currentIdx === 0 ? displayItems.length - 1 : currentIdx - 1;
      } else {
        newIdx = currentIdx === displayItems.length - 1 ? 0 : currentIdx + 1;
      }

      setSelectedItem(displayItems[newIdx]);
    },
  ...
  const handleBulkDelete = useCallback(async () => {
    ...
    try {
      ...
      setTotalItems((prev) => Math.max(0, prev - remoteIds.length));
    ...
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="page-header px-4 md:px-6 py-4 pt-14 md:pt-4 animate-in fade-in slide-in-from-bottom-2 duration-300 fill-mode-both">
        ...
        <div className="flex flex-wrap items-center gap-2 md:gap-3">
          ...
          {isSelectionMode && displayItems.length > 0 && (
            <>
              <Button
                variant="outline"
                size="sm"
                onClick={handleSelectAll}
                disabled={selectedIds.size === displayItems.length}
              >
                {t("common.selectAll")}
              </Button>
              <Button
