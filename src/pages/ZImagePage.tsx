...
import { persistPlatformMediaUrl } from "@/api/platformMedia";
...
export function ZImagePage() {
  ...
  const handleGenerate = async () => {
    ...
    if (!electronAvailable) {
      setError(
        t(
          ...
          "This feature requires the desktop app. Please download 造梦影视与设计平台 or try the online version.",
        ),
      );
      return;
    }
    ...
    try {
      ...
      const localImageUrl = `local-asset://${encodeURIComponent(result.outputPath)}`;
      const platformFile = await persistPlatformMediaUrl(localImageUrl, {
        filename: `z-image_${predictionId}.png`,
      });
      const imageUrl = platformFile.url;
