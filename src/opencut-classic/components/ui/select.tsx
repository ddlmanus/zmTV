...
const selectItemVariants = cva(
  "relative flex cursor-pointer select-none items-center gap-1.5 rounded-sm px-2 py-1 text-sm text-[#d7d9dd] outline-hidden data-[highlighted]:bg-[#2f3033] data-[highlighted]:text-white data-disabled:pointer-events-none data-disabled:opacity-50 [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
        default: "",
...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        "z-50 max-h-(--radix-select-content-available-height) min-w-32 overflow-hidden rounded-md border border-[#2b2c2f] bg-[#1b1c1f] p-1 text-[#d7d9dd] shadow-[0_16px_40px_rgba(0,0,0,0.45)]",
        className,
      )}
      position={position}
      onCloseAutoFocus={(e) => {
...
>(({ className, ...props }, ref) => (
  <SelectPrimitive.Label
    ref={ref}
    className={cn(
      "px-2 pb-1 pt-0.5 text-[11px] font-bold uppercase tracking-wider text-[#8d929c]",
      className,
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <SelectPrimitive.Separator
    ref={ref}
    className={cn("mx-1 my-1 h-px bg-[#2b2c2f]", className)}
    {...props}
  />
));
