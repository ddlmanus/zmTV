...
import type { EffectElement, VisualElement } from "@/opencut-classic/timeline";
import { effectsRegistry } from "@/opencut-classic/effects";
...
import { PropertyParamField } from "@/opencut-classic/components/editor/panels/properties/components/property-param-field";
	...
	return (
		<div className="flex flex-col h-full">
			<div className="border-b px-3.5 h-11 shrink-0 flex items-center">
				<SectionTitle>{t("freeTools.mediaTrimmer.editor.effect")}</SectionTitle>
			</div>
			<EffectSection
				effect={effect}
...
export function ClipEffectsTab({
	...
	return (
		<div className="flex flex-col h-full">
			<div className="border-b px-3.5 h-11 shrink-0 flex items-center">
				<SectionTitle>{t("freeTools.mediaTrimmer.editor.effects")}</SectionTitle>
			</div>
			{effects.length === 0 ? (
				<EmptyView />
			) : (
		...
		<div className="flex flex-col h-full items-center justify-center gap-4 text-center">
			...
			<div className="flex flex-col gap-2">
				<h3 className="font-medium text-foreground">
					{t("freeTools.mediaTrimmer.editor.noEffects")}
				</h3>
				<p className="text-muted-foreground text-sm text-balance max-w-44">
					{t("freeTools.mediaTrimmer.editor.addEffectsHint")}
				</p>
			</div>
			<Button
				variant="default"
			...
			>
				{t("freeTools.mediaTrimmer.editor.openEffects")}
			</Button>
		</div>
	);
}
					...
					onToggle && (
						<div className="flex items-center gap-1">
							<Button
								...
								size="icon"
								aria-label={t("freeTools.mediaTrimmer.editor.toggleEffect", {
									name: effectName,
								})}
								onClick={onToggle}
							>
							...
							<Button
								variant="ghost"
								size="icon"
								aria-label={t("freeTools.mediaTrimmer.editor.removeEffect", {
									name: effectName,
								})}
								onClick={onRemove}
							>
