...
import type {
  ...
  TProjectSettings,
  TTimelineViewState,
} from "@/opencut-classic/project/types";
import type {
  ExportOptions,
  ExportResult,
  ExportState,
...
import { UpdateProjectSettingsCommand } from "@/opencut-classic/commands/project";
import { DEFAULT_BACKGROUND_COLOR } from "@/opencut-classic/background/color";
import { DEFAULT_CANVAS_SIZE } from "@/opencut-classic/canvas/sizes";
...
} from "@/opencut-classic/services/storage/migrations";
import { loadFonts } from "@/opencut-classic/fonts/google-fonts";
import { DEFAULTS } from "@/opencut-classic/timeline/defaults";
import { getElementFontFamilies } from "@/opencut-classic/timeline/element-utils";
import { getRaisedProjectFpsForImportedMedia } from "@/opencut-classic/fps/utils";
...
export class ProjectManager {
  ...
  async createNewProject({ name }: { name: string }): Promise<string> {
    const mainScene = buildDefaultScene({
      name: i18n.t("freeTools.mediaTrimmer.editor.mainScene"),
      isMain: true,
    });
