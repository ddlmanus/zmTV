...
import type { MaskableElement } from "@/opencut-classic/timeline";
import type { Mask, MaskType, TextMask } from "@/opencut-classic/masks/types";
import type { NumberParamDefinition, SelectParamDefinition } from "@/opencut-classic/params";
import {
	buildDefaultMaskInstance,
	getMaskDefinition,
	getMaskDefinitionsForMenu,
} from "@/opencut-classic/masks";
...
import { useMenuPreview } from "@/opencut-classic/editor/use-menu-preview";
import { getVisibleElementsWithBounds } from "@/opencut-classic/preview/element-bounds";
...
import { FontPicker } from "@/opencut-classic/components/ui/font-picker";
import {
	DropdownMenu,
	DropdownMenuContent,
	DropdownMenuItem,
...
import {
	...
	OcTextHeightIcon,
	OcTextWidthIcon,
} from "@/opencut-classic/components/icons";
	...
	return (
		<div className="flex flex-col h-full">
			<div className="border-b px-3.5 h-11 shrink-0 flex items-center justify-between gap-2">
				<SectionTitle>{t("freeTools.mediaTrimmer.editor.masksPanel.title")}</SectionTitle>
				<DropdownMenu
					...
					{hasMask ? (
						<Tooltip>
							<TooltipTrigger asChild>
								<span className="inline-flex">
									<Button
										...
										size="icon"
										disabled
										aria-label={t("freeTools.mediaTrimmer.editor.masksPanel.add")}
									>
							...
							<TooltipContent className="max-w-56 text-balance">
								{t("freeTools.mediaTrimmer.editor.masksPanel.singleMaskHint")}
							</TooltipContent>
						</Tooltip>
					) : (
					...
					<DropdownMenuContent className="w-40" onPointerLeave={onPointerLeave}>
						{maskDefs.map((definition) => (
							...
							>
								...
								{t(`freeTools.mediaTrimmer.editor.maskNames.${definition.type}`, {
...
function MaskItem({
	...
	const maskName = t(`freeTools.mediaTrimmer.editor.maskNames.${definition.type}`, {
		defaultValue: definition.name,
	});
				...
				trailing={
					<div className="flex items-center gap-1">
						<Button
							variant="ghost"
							size="icon"
							aria-label={t("freeTools.mediaTrimmer.editor.masksPanel.toggleInvert", {
								name: maskName,
							})}
							onClick={() =>
						...
						<Button
							variant="ghost"
							size="icon"
							aria-label={t("freeTools.mediaTrimmer.editor.masksPanel.remove", {
								name: maskName,
							})}
							onClick={() =>
		...
		<div className="flex flex-col h-full items-center justify-center gap-4 text-center">
			...
			<div className="flex flex-col gap-2">
				<h3 className="font-medium text-foreground">
					{t("freeTools.mediaTrimmer.editor.masksPanel.emptyTitle")}
				</h3>
				<p className="text-muted-foreground text-sm text-balance max-w-40">
					{t("freeTools.mediaTrimmer.editor.masksPanel.emptyHint")}
				</p>
			</div>
			<Button variant="default" size="sm" onClick={onAddMask}>
				{t("freeTools.mediaTrimmer.editor.masksPanel.add")}
			</Button>
		</div>
	);
}
