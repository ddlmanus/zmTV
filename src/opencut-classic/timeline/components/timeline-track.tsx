...
import type { TimelineElement as TimelineElementType } from "@/opencut-classic/timeline";
import { TIMELINE_LAYERS } from "./layers";
import type { ElementDragView } from "@/opencut-classic/timeline";
...
export function TimelineTrackContent({
  ...
  const trackLabel = getLocalizedTrackName({ name: track.name, t });
  ...
  return (
    <div className="relative size-full">
      <button
        ...
        aria-label={t("freeTools.mediaTrimmer.editor.selectTrack", {
          track: trackLabel,
        })}
        onMouseUp={(event) => {
...
function getLocalizedTrackName({
  name,
  t,
}: {
  name: string;
  ...
  const defaultTrackKeyByName: Record<string, string> = {
    "Main Track": "mainTrack",
    "Video track": "videoTrack",
    "Text track": "textTrack",
    "Audio track": "audioTrack",
    "Graphic track": "graphicTrack",
    "Effect track": "effectTrack",
  };
  const key = defaultTrackKeyByName[name];
  return key ? t(`freeTools.mediaTrimmer.editor.${key}`) : name;
}
