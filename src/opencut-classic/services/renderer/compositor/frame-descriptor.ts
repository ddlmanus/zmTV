import { drawCssBackground } from "@/opencut-classic/gradients";
import { getMaskDefinition } from "@/opencut-classic/masks";
import { incrementCounter } from "@/opencut-classic/diagnostics/render-perf";
...
import {
	applyCpuEffectPassesToContext,
	partitionEffectPassGroups,
...
async function collectNode({
	...
	if (node instanceof EffectLayerNode) {
		...
		const { gpuEffectPassGroups } = partitionEffectPassGroups({
			effectPassGroups: [node.resolved.passes],
		});
		if (gpuEffectPassGroups.length === 0) {
			return;
		}
		items.push({
			type: "sceneEffect",
			effectPassGroups: gpuEffectPassGroups,
		});
		return;
	}
...
async function collectVisualSourceNode({
	...
	const { cpuPasses, gpuEffectPassGroups } = partitionEffectPassGroups({
	...
	if (cpuPasses.length > 0) {
		textures.set(textureId, {
			kind: "rendered",
			id: textureId,
			contentHash: `source-cpu:${identityKey(source)}:${sourceWidth}x${sourceHeight}:${JSON.stringify(cpuPasses)}`,
			width: sourceWidth,
			height: sourceHeight,
			draw: (ctx) => {
