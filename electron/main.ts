...
import { registerApiProxyHandlers } from "./apiProxy";
import {
  handleWorkflowBackendRequest,
  startWorkflowBackend,
  stopWorkflowBackend,
  type WorkflowBackendProviderConfig,
} from "./workflow-backend/server";
import {
  disposeWorkflowPlatformMediaBroker,
  registerWorkflowPlatformMediaBroker,
} from "./workflow-backend/platform-media-broker";
import log from "electron-log";

const EXTRACT_FRAME_DEBUG = process.env.WAVESPEED_EXTRACT_FRAME_DEBUG === "1";
const REMOTE_ASSET_URL_PREFIX = "local-asset://remote/";
...
async function remoteAssetResponse(request: Request): Promise<Response> {
  try {
    const sourceUrl = decodeURIComponent(
      request.url.slice(REMOTE_ASSET_URL_PREFIX.length),
    );
    const parsedSource = new URL(sourceUrl);
    if (
      !["http:", "https:"].includes(parsedSource.protocol) ||
      parsedSource.username ||
      parsedSource.password
    ) {
      return new Response("Invalid remote asset URL", { status: 400 });
    }
    ...
    const upstream = await net.fetch(parsedSource.toString(), { headers });
    ...
    return new Response(
      error instanceof Error ? error.message : "Remote asset request failed",
      { status: 502 },
    );
  }
}

async function localAssetResponse(request: Request): Promise<Response> {
  if (request.url.startsWith(REMOTE_ASSET_URL_PREFIX)) {
    return remoteAssetResponse(request);
  }
  ...
  return new Response(Uint8Array.from(readFileSync(filePath)), {
    status: 200,
    headers: {
      "Accept-Ranges": "bytes",
...
function loadSettings(): Settings {
  try {
    if (existsSync(settingsPath)) {
      ...
      const apiKeys = { ...(rawSettings.apiKeys || {}) };
      if (rawSettings.apiKey && !apiKeys[rawSettings.apiServiceId]) {
        apiKeys[rawSettings.apiServiceId] = rawSettings.apiKey;
      }
      if (apiKeys.custom && !apiKeys["one-api"]) {
        apiKeys["one-api"] = apiKeys.custom;
      }
      rawSettings.apiKeys = apiKeys;
...
ipcMain.handle("set-api-key", (_, apiKey: string, credentialScope?: string) => {
  ...
  const targetScope = resolveCredentialScope(credentialScope, settings);
  saveSettings({
    apiKeys: { ...settings.apiKeys, [targetScope]: apiKey },
  });
  return true;
});
...
function workflowCodexProviderConfig(): WorkflowBackendProviderConfig {
  const settings = loadSettings();
...
function workflowPlatformProviderConfig(): WorkflowBackendProviderConfig {
  const settings = loadSettings();
  const apiKey = String(
    settings.apiKeys["zaomeng-api"] ||
      (settings.apiServiceId === "zaomeng-api" ? settings.apiKey : "") ||
      "",
  ).trim();
  return {
    baseUrl: ZAOMENG_OPEN_API_BASE_URL + "/v1",
...
app.whenReady().then(async () => {
  ...
  protocol.handle("zaomeng-workflow", (request) => {
    return handleWorkflowBackendRequest(request);
  });
  ...
  try {
    await startWorkflowBackend({
      appRoot,
      resourcesRoot,
      runtimeRoot: join(app.getPath("userData"), "workflow-runtime"),
      getProviderConfig: workflowCodexProviderConfig,
      getPlatformProviderConfig: workflowPlatformProviderConfig,
      fetchRemote: (input, init) =>
        net.fetch(input instanceof URL ? input.toString() : input, init),
    });
  } catch (error) {
    console.error("Failed to start workflow backend:", error);
  }
  ...
  createWindow();
  registerWorkflowPlatformMediaBroker(() => mainWindow);
