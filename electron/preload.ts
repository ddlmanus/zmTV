...
import type { ApiProxyRequest, ApiProxyResponse } from "../src/types/apiProxy";
import type {
...
interface CanvasAssetGroupItemInput {
...
interface CanvasAssetGroupInput {
...
const electronAPI = {
  apiProxyRequest: (request: ApiProxyRequest): Promise<ApiProxyResponse> =>
    ipcRenderer.invoke("api-proxy:request", request),
  cancelApiProxyRequest: (requestId: string): void =>
    ipcRenderer.send("api-proxy:cancel", requestId),
  onWorkflowPlatformMediaRequest: (
    callback: (request: WorkflowPlatformMediaRequest) => void,
  ): (() => void) => {
    const handler = (_: unknown, request: WorkflowPlatformMediaRequest) =>
      callback(request);
    ipcRenderer.on("workflow-platform-media:request", handler);
    return () =>
      ipcRenderer.removeListener("workflow-platform-media:request", handler);
  },
  resolveWorkflowPlatformMediaRequest: (
    requestId: string,
    response: WorkflowPlatformMediaResponse,
  ): void =>
    ipcRenderer.send("workflow-platform-media:response", requestId, response),
  getApiKey: (credentialScope?: string): Promise<string> =>
    ipcRenderer.invoke("get-api-key", credentialScope),
  setApiKey: (apiKey: string, credentialScope?: string): Promise<boolean> =>
    ipcRenderer.invoke("set-api-key", apiKey, credentialScope),
  ...
  saveCanvasAssetGroup: (
    ...
    ipcRenderer.invoke("canvas-asset-group:save", payload),
  getCanvasAssetGroup: (
    id: string,
  ): Promise<{ group: unknown; items: unknown[] } | null> =>
    ipcRenderer.invoke("canvas-asset-group:get", { id }),
  listCanvasAssetGroups: (limit?: number): Promise<unknown[]> =>
    ipcRenderer.invoke("canvas-asset-group:list", { limit }),
};
