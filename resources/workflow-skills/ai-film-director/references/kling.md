...
- `generate_audio` — bool. There is no reliable "no music" toggle. Keep native audio enabled; when a shot requires dialogue, ambience, or effects without music, route it to a compatible native-audio model or mode. Only adjust or replace an unsuitable background-music layer after review, without discarding usable dialogue, ambience, or effects.
