...
- Transcribe and map its exact lines/timing into the storyboard. Prefer a native-audio video model or an audio-reference mode that can honor the supplied performance; do not declare a silent cut complete and defer the core voice track downstream.

If no VO: default to an audible native track with planned ambience, interaction sounds, and suitable music intent. The first three seconds should remain visually understandable when a playback platform starts muted, but the generated asset itself stays audible.

### 8. Soundtrack

**Optional replacement or user-specified input.** Native-audio video generation is the default. Choose a licensed external track only when the user requires a specific score or the native background music fails review. Recommended sources:
...
- Sound plan ✅ (no fallback: storyboard ambience, interaction sounds, music intent, and caption timing; a separate soundtrack file is optional)
...
- VO: none (native ambience, interaction sounds, and music enabled)
- Soundtrack: optional; use public/soundtrack.mp3 only when specified or approved as a replacement
- Reference videos:
