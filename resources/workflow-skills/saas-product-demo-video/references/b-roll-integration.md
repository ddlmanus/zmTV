...
- Several audible b-roll layers would overlap at the same time. Keep one active native track, or plan an explicit background-music replacement after review.
...
export const BRollLayer: React.FC<BRollLayerProps> = ({
  ...
  fadeIn = 6, fadeOut = 6, windowEnd, muteNativeAudio = false,
}) => {
...
- **Native audio is preserved by default.** For AI-generated b-roll, keep `muteNativeAudio={false}`. Set it to `true` only after review when that layer's background music conflicts with an approved replacement; do not discard usable dialogue, ambience, or effects by default.
