...
> **造梦 integration override:** do not execute the Gemini CLI, Gemini API, API-key,
> curl-wrapper, or AI Studio paths preserved below from the upstream reference. In 造梦,
> inspect the reference with the current Codex/browser capabilities or extract representative
> frames with ffmpeg and analyze those locally. The bundled `analyze-reference-video.sh` is
> intentionally disabled. Never request a supplier API key.
