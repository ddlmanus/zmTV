...
1. 真实产品页面、组件和交互状态：使用 Playwright/Puppeteer 或桌面截图采集。
2. 镜头运动、文字、转场、声音和最终成片：使用 Remotion/ffmpeg 本地合成。
3. AI 生成的 styleframe、背景、补充图片、B-roll 或过场视频：只使用
   `$CODEX_HOME/skills/.system/platform-media/scripts/platform_media.py` 的
   `image` / `video` 命令，经 造梦系统模型配置、默认参数、积分和持久化链路执行。

禁止直接调用供应商 API、SDK、CLI 或供应商密钥。Platform Media 失败、无可用模型
或积分不足时，停止对应生成步骤并报告真实错误；不得切换到系统外供应商。系统生成
的素材必须拿到真实输出并写入当前项目后，才能进入逐镜头实现和最终渲染。
