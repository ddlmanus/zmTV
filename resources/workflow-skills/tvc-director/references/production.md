...
通过造梦 Platform Media 的参考图生成链路传入资产图：
- 第一层正常写全局风格
...
通过造梦 Platform Media 传入多宫格原图作为构图参考，按阅读顺序逐格生成独立关键帧；同时传入产品或角色身份参考，避免网格拆分造成身份漂移。
...
**必须覆盖**：视觉风格、色彩基调、约束条件、**原生同步声音与音乐意图**、需要字幕时的原文和起止时间、**产品@产品多视图图片**，并确保请求启用 `generateAudio=true`。
...
> **音频铁律**：每镜在分镜阶段写清角色对白/旁白、环境声、动作拟音、音乐意图与静默区，由视频模型原生生成同步声轨。多段拼接时先保留验收合格的原生对白、环境声和拟音；只有背景音乐明显不合适或接缝跳变时才替换/调整 BGM。角色对白在镜头描述中用引号标注。

> **区分模型指令与确定性叠加**：视频提示词必须同时描述模型要生成的画面、对白/旁白、环境声、拟音、音乐意图，以及需要展示的字幕原文和起止时间。Logo/品牌标识、Slogan、价格、法务文字和最终调色仍属于确定性后期，不写成模型要想象的内容。End Frame 在视频提示词中描述产品定格、画面留白和声音收束，Logo/Slogan 的空间预留由多宫格图片提示词负责。

**输出示例**：
> 风格：暗黑童话告别场景 / 红黑高对比森林色调 / 定格动画质感 / 强烈光源对冲构图（门内白光 vs 门外赤红森林）/ 细腻表情特写 / 情绪克制的离别氛围 / 原生同步林间环境声、脚步和克制告别配乐 / 本镜无对白因此不显示字幕 产品@产品多视图图片 的广告
